...
from .serializers import UserSerializer, GroupSerializer, CaseSerializer, ResultSerializer, FlowSerializer, \
    SuiteSerializer, AuthSerializer, AuthScript1Serializer, PlanScriptSerilizer,UserInfoSerializer
...
#log.initLogging('/data/server/log')
...
class UserInfoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if name:
            return UserInfo.objects.filter(Q(name=name)).order_by('id')
        elif address:
            return UserInfo.objects.filter(Q(address=address)).order_by('id')
        else:
